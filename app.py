...
with st.sidebar:
    ...
    temp_change_slider = st.slider(
        ...
        value=0.0,
        step=0.5,
        help="Ujemne wartości (-) to redukcja temperatury, dodatnie (+) to podwyższenie.",
    )
    ...
    if temp_change_slider < 0:
        st.success(
            f"💡 Obniżenie o **{abs(temp_change_slider)}°C** da ok. **{abs(simulated_diff_units):.1f} U** oszczędności (~**{abs(simulated_diff_pln):.2f} PLN** w sezonie)."
        )
    elif temp_change_slider > 0:
        st.error(
            f"⚠️ Podwyższenie o **+{temp_change_slider}°C** zwiększy zużycie o ok. **+{simulated_diff_units:.1f} U** (~**+{simulated_diff_pln:.2f} PLN** w sezonie)."
        )
    else:
    ...
    df_base_sum = (
        df[df["season"].str.contains("Bazowy", na=False)]["delta_units"].sum()
        if not df.empty
        else 0.0
    )
    total_saved_units = (
        max(0.0, df_base_sum - total_apartment_units) if df_base_sum > 0 else 0.0
    )
    total_saved_pln = total_saved_units * EST_PLN_PER_UNIT

    payback_ratio = (
        min(1.0, total_saved_pln / HARDWARE_COST_EST)
        if HARDWARE_COST_EST > 0
        else 1.0
    )
    st.caption(
        f"Koszt zakupu sprzętu Sonoff: **~{HARDWARE_COST_EST:.0f} PLN**"
    )
    st.progress(
        payback_ratio,
        text=f"Spłacono: {total_saved_pln:.2f} PLN ({payback_ratio*100:.1f}%)",
    )

    # WSKAŹNIK BUFORA BEZPIECZEŃSTWA
    weekly_avg_cost = (
        total_realtime_cost / weeks_logged_count if weeks_logged_count > 0 else 1.0
    )
    buffer_weeks = (
        max(0.0, ssm_net_balance_to_date / weekly_avg_cost)
        if weekly_avg_cost > 0
        else 0.0
    )
    st.caption(
        f"🛡️ Bufor bezpiecznego grzania z zaliczek: **{buffer_weeks:.1f} tyg.**"
    )
    ...
    with st.expander("🛠️ Panel Debug & Opcje Zaawansowane"):
        ...
        if st.button(
            "🔄 Reset bazy do stanu początkowego", use_container_width=True
        ):
...
with tab_charts:
    if current_room == "Licznik Główny":
        st.markdown(
            "#### Korelacja Zużycia Licznika Głównego (Suma Stref [U] oraz Energia [GJ])"
        )
    else:
        st.markdown(
            f"#### Korelacja Zużycia ΔU oraz Temperatury Zewnętrznej dla: **{current_room}**"
        )
    ...
    st.markdown("---")
    st.markdown(
        "#### 🏗️ Struktura Zużycia w Całym Mieszkaniu (Wykres Skumulowany Pokoi)"
    )
    st.caption(
        "Wyjaśnienie: Wykres przedstawia tygodniowy udział każdego pokoju. Ich łączna wysokość daje pełny wynik Licznika Głównego."
    )
...
with tab_season_comp:
    st.markdown(
        "### 📊 Porównanie Sezonów, Koszt Narastający & Linie Zaliczek SSM"
    )
    st.caption(
        "Wyjaśnienie: Linia przerywana przedstawia zgromadzone wpłaty zaliczek do Spółdzielni od 1 września. Przebieg zużycia poniżej tej linii oznacza zwrot gotówki."
    )
    ...
    if not df.empty:
        ...
        with col_sc1:
            df_comp = (
                df[df["room_name"] != "Licznik Główny"]
                .groupby(["week_num", "season"])["delta_units"]
                .sum()
                .reset_index()
            )
            fig_season = px.line(
                ...
                color="season",
                markers=True,
                title="Zużycie tygodniowe: Obecny Sezon vs Bazowy",
                labels={
                    "week_num": "Tydzień Roku",
                    "delta_units": "Suma Mieszkania [U]",
                    "season": "Sezon",
                },
            )
        ...
        with col_sc2:
            df_cum = (
                df[df["room_name"] != "Licznik Główny"]
                .groupby(["season", "week_num"])["delta_units"]
                .sum()
                .groupby(level=0)
                .cumsum()
                .reset_index()
            ...
            # Linia zaliczek SSM (sezon 35 tygodni)
            weeks_seq = list(df_cum["week_num"].unique())
            ssm_advances_line = [
                (w / SSM_TOTAL_WEEKS) * SSM_ANNUAL_CO_BUDGET for w in weeks_seq
            ]
            fig_cum.add_trace(
                px_go.Scatter(
                    x=weeks_seq,
                    y=ssm_advances_line,
                    name="💰 Skumulowane Wpłaty Zaliczek SSM",
            ...
            fig_cum.update_layout(
                ...
                yaxis_title="Koszt Narastający [PLN]",
                template="plotly_white",
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                height=360,
        ...
        # ---------------------------------------------------------
        # WYKRES INTERAKTYWNY: KOSZTY CO VS OPŁATY CAŁKOWITE (CZYNSZ SSM)
        # ---------------------------------------------------------
        st.markdown("---")
        st.markdown(
            "#### 🏢 Struktura Opłat Całkowitych (Czynsz SSM vs Rzeczywisty Koszt CO)"
        )
        st.caption(
            f"Porównanie opłat stałych (eksploatacja, woda, fundusz: **{SSM_NON_HEATING_RENT:.2f} PLN**), zaliczki na CO (**{SSM_CO_MONTHLY_ADVANCE:.2f} PLN**) oraz **rzeczywistego poboru ciepła Sonoff** od 1 września."
        )
        ...
        for w in weeks_in_data:
            w_units = (
                df_sonoff_all[df_sonoff_all["week_num"] == w][
                    "delta_units"
                ].sum()
                if not df_sonoff_all.empty
                else 0.0
            ...
            w_co_advance = (
                SSM_CO_MONTHLY_ADVANCE / 4.375
            )  # Przelicznik tygodniowy (35 tyg / 8 mc)
            w_fixed_rent = SSM_NON_HEATING_RENT / 4.375
            ...
            chart_data_rent.append({
                ...
                "Udział CO w Czynszu (%)": round(
                    (w_co_cost_real / w_total_rent_actual * 100)
                    if w_total_rent_actual > 0
                    else 0,
                    1,
                ),
            })
        ...
        with col_r1:
            ...
            fig_total_fees.add_trace(
                px_go.Bar(
                    x=df_rent_chart["Tydzień"],
                    y=df_rent_chart["Opłaty Stałe (Czynsz bez CO)"],
                    name="Opłaty Stałe SSM (Eksploatacja/Woda)",
                    marker_color="#8E8E93",
                )
            )
            fig_total_fees.add_trace(
                px_go.Bar(
                    x=df_rent_chart["Tydzień"],
                    y=df_rent_chart["Rzeczywiste CO (Sonoff)"],
                    name="Rzeczywisty Koszt CO (Sonoff)",
                    marker_color="#007AFF",
                )
            )

            # Linia pobieranego czynszu z zaliczką SSM
            fig_total_fees.add_trace(
                px_go.Scatter(
                    x=df_rent_chart["Tydzień"],
                    y=df_rent_chart["Czynsz Całkowity z Zaliczką"],
                    name="Wpłacany Czynsz Całkowity (z Zaliczką CO)",
                    mode="lines+markers",
        ...
        with col_r2:
            avg_co_real = (
                df_rent_chart["Rzeczywiste CO (Sonoff)"].mean()
                if not df_rent_chart.empty
                else 0.0
            )
            fixed_rent_weekly = SSM_NON_HEATING_RENT / 4.375
...
with tab_analytics:
    ...
    if not df.empty:
        ...
        df_base = df[
            df["season"].str.contains("Bazowy", na=False)
            & (df["room_name"] != "Licznik Główny")
        ].copy()

        df_weekly = (
            df_sonoff.groupby(["week_num", "period_label"])["delta_units"]
            .sum()
            .reset_index()
        )
        df_weekly_base = (
            df_base.groupby(["week_num"])["delta_units"]
            .sum()
            .reset_index()
            .rename(columns={"delta_units": "base_units"})
        )

        df_merged = pd.merge(
            df_weekly, df_weekly_base, on="week_num", how="left"
        ).fillna(0)
        df_merged["units_saved"] = (
            df_merged["base_units"] - df_merged["delta_units"]
        )
        df_merged["pln_balance"] = df_merged["units_saved"] * EST_PLN_PER_UNIT
        df_merged["color"] = df_merged["pln_balance"].apply(
            lambda x: "#34C759" if x >= 0 else "#FF3B30"
        )
        ...
        with col_b:
            st.markdown("#### 🍩 Podział Zużycia według Trybów Grzania")
            if not df_sonoff.empty:
                df_mode_pie = (
                    df_sonoff.groupby("mode_tag")["delta_units"]
                    .sum()
                    .reset_index()
                )
                fig_pie = px.pie(
        ...
        st.markdown("---")
        st.markdown(
            "#### 🔥 Macierz Korelacji (Temperatura Zewnętrzna vs Zużycie Mieszkania)"
        )
        if len(df_sonoff) >= 2:
            fig_heat = px.density_heatmap(
                ...
                z="delta_units",
                histfunc="avg",
                title="Intensywność poboru energii w zależności od temperatury na zewnątrz",
                labels={
                    "temp_zewnetrzna": "Temp. Zewnętrzna [°C]",
                    "delta_units": "Średnie Zużycie Strefy [U]",
                },
            )
...
with tab_ai_pred:
    st.markdown(
        "### 🤖 Predykcja AI, Wykrywanie Anomalii & Prędkościomierz Budżetowy SSM"
    )
    ...
    if not df_room_sonoff.empty and len(df_room_sonoff) >= 2:
        ...
        if (
            prev_u > 0
            and (last_u - prev_u) / prev_u > 0.25
            and last_temp >= prev_temp
        ):
    ...
    with col_p1:
        ...
        if anomaly_detected:
            st.error(
                f"⚠️ **Alert Inteligentnego Wykrywania Anomalii:**\n\n{anomaly_msg}"
            )
        else:
            st.success(
                "✅ **Stan Normalny (Brak Anomalii):**\n\nZużycie we wszystkich strefach jest stabilne i współgra z aktualną pogodą w Bytkowie."
            )

        st.info(
            "💡 **Szczegółowy Raport Rozliczeniowy ze Spółdzielnią (1 Września - 30 Kwietnia):**\n\n"
            f"- Łączny budżet zaliczkowy CO na 8 miesięcy (35 tygodni) wynosi **{SSM_ANNUAL_CO_BUDGET:.2f} PLN**.\n"
            f"- Przy aktualnym tempie całkowity koszt wyniesie **~{ssm_projected_full_season_cost:.2f} PLN**.\n"
            f"- Przewidywany wynik końcowy rozliczenia z SSM na koniec kwietnia: **{'ZWROT +' if ssm_projected_refund_or_extra>=0 else 'DOPŁATA '}{ssm_projected_refund_or_extra:.2f} PLN**."
        )

    with col_p2:
        st.markdown("#### ⏱️ Prędkościomierz Wykorzystania Zaliczki SSM")
        fig_gauge = px_go.Figure(
            px_go.Indicator(
                mode="gauge+number+delta",
                value=total_realtime_cost,
                domain={"x": [0, 1], "y": [0, 1]},
                title={
                    "text": "Zużycie Budżetu CO (PLN)",
                    "font": {"size": 16},
                },
                delta={
                    "reference": ssm_paid_advances_to_date,
                    "increasing": {"color": "red"},
                },
                gauge={
                    "axis": {
                        "range": [None, SSM_ANNUAL_CO_BUDGET],
                        "tickwidth": 1,
                        "tickcolor": "darkblue",
                    },
                    "bar": {"color": "#007AFF"},
                    ...
                    "steps": [
                        {
                            "range": [0, SSM_ANNUAL_CO_BUDGET * 0.6],
                            "color": "rgba(52, 199, 89, 0.2)",
                        },
                        {
                            "range": [
                                SSM_ANNUAL_CO_BUDGET * 0.6,
                                SSM_ANNUAL_CO_BUDGET * 0.9,
                            ],
...
with tab_history:
    ...
    st.caption(
        "Wyjaśnienie: Zbiór wszystkich wtorkowych odczytów zebranych z podzielników oraz licznika głównego od 1 września."
    )
    ...
    if df.empty:
        ...
        st.write(
            "Zaznacz wiersze w kolumnie **Zaznacz**, aby je usunąć, lub pobierz historię do pliku CSV."
        )
