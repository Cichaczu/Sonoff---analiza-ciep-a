...
# ---------------------------------------------------------
    ...
    # Przygotowanie danych dla wybranej strefy
    if current_room == "Licznik Główny":
        df_filtered = df.copy()
    else:
        df_filtered = df[df["room_name"] == current_room] if not df.empty else pd.DataFrame()

    # SPRAWDŹ CZY DZIŚ JEST WTOREK (INDYWIDUALNY FORMULARZ DLA AKTYWNEJ STREFY)
    today = date.today()
    ...
    if is_tuesday and current_room != "Licznik Główny":
        ...
        if not df_filtered.empty:
            already_added_this_week = ((df_filtered["week_num"] == current_iso_w) & (df_filtered["season"] == "2026/2027 (Sonoff - Wtorki)")).any()
        ...
        if not already_added_this_week:
            with st.expander(f"🚨 WTOREK – Wymagany Odczyt dla strefy: {current_room} (Tydzień {current_iso_w})", expanded=True):
                st.warning(f"Dziś jest wtorek! Wprowadź dzisiejszy odczyt z podzielnika dla pomieszczenia: **{current_room}**.")
                
                # Pobierz ostatni stan dla tego konkretnego pokoju
                last_row = df_filtered.iloc[-1] if not df_filtered.empty else None
                default_units_start = float(last_row["units_end"]) if last_row is not None else ROOMS_CONFIG[current_room]["units_start"]
                default_meter = str(last_row["meter_number"]) if last_row is not None else ROOMS_CONFIG[current_room]["meter_default"]
    ...
    # ODŚWIEŻENIE FILTROWANIA PO EWENTUALNYM ZAPISIE
    if current_room == "Licznik Główny":
        df_filtered = df.copy()
    else:
        df_filtered = df[df["room_name"] == current_room] if not df.empty else pd.DataFrame()
